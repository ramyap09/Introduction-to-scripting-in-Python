...
def filter_by_year(statistics, year, yearid):
    ...
    """
    year_filtered_stat = list(filter(lambda subdict : int(subdict[yearid]) == year, statistics))
    #print(year_filtered_stat)
    return year_filtered_stat
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    """
    list_of_tups = list(map(lambda x:(x[info["playerid"]],formula(info, x)),statistics))
    list_of_tups.sort(key = lambda tup:tup[1] , reverse=True)
    return list_of_tups[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    for tup in top_ids_and_stats:
        for dic in statistics:
            if dic[info["playerid"]] == tup[0]:
                strng = "{:4.3f}".format(tup[1]) \
                + " --- " + dic[info["firstname"]] + " " + dic[info["lastname"]]
                lst.append(strng)
    return lst
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    filtered_statistics = filter_by_year(statistics, year, info["yearid"])
    top_ids_and_stats = top_player_ids(info, filtered_statistics, formula, numplayers)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for subdict in statistics:
        if subdict[playerid] not in newdict :
            newdict[subdict[playerid]] = {x: int(subdict[x]) for x in fields}
            newdict[subdict[playerid]][playerid] = subdict[playerid]
        else:
            newdict[subdict[playerid]] ={ x : (int(newdict[subdict[playerid]][x]) \
            + int(subdict[x])) for x in fields }
            newdict[subdict[playerid]][playerid] = subdict[playerid]
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    """
    statistics = read_csv_as_list_dict(info["battingfile"], info["separator"], info["quote"])
    agg_player = aggregate_by_player_id(statistics, info["playerid"], info["battingfields"])
    mod_statistics = agg_player.values()
    t_p_id = top_player_ids(info, mod_statistics, formula, numplayers)
    result = lookup_player_names(info, t_p_id)
...
def test_baseball_statistics():
    ...
    #
    baseballdatainfo = {"masterfile": "master2.csv",   # Name of Master CSV file
